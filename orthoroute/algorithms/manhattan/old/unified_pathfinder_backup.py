...
@dataclass
class EdgeRec:
    """Unified edge record for PathFinder accounting (canonical + CSR compatibility)"""
    # Canonical edge store attributes (new system)
    owner_net: Optional[str] = None
    taboo_until_iter: int = 0
    historical_cost: float = 0.0

    # CSR system attributes (legacy compatibility)
    usage: int = 0
    owners: set = field(default_factory=set)  # net_ids currently committed
    pres_cost: float = 1.0    # present/congestion multiplier
    hist_cost: float = 0.0    # accumulated history (legacy)
...
class UnifiedPathFinder:
    ...
    def _calculate_iteration_metrics(self, successful: int, failed_nets: int, routes_changed: int,
        ...
        for edge_key, edge_rec in self.edge_store.items():
            ...
            # Handle both new canonical EdgeRec and legacy EdgeRec
            if hasattr(edge_rec, 'owner_net'):
                # New canonical EdgeRec with single owner
            ...
            elif hasattr(edge_rec, 'owners'):
                # Legacy EdgeRec with multiple owners (fallback)
                if edge_rec.owners:
                    # Edge has owners - count as utilized
                    pass
                history_total += edge_rec.hist_cost
            else:
                # Unknown EdgeRec type - skip safely
                continue
    ...
    def _build_geometry_intents(self):
        """Build geometry intents from CANONICAL EDGE STORE - authoritative source."""
        ...
        # CRITICAL FIX: Build geometry directly from canonical edge store
        # This prevents track-track violations by using authoritative edge ownership
        kept_segments = 0
        processed_edges = set()

        logger.info(f"[CANONICAL-EMIT] Processing {len(self.edge_store)} edges from canonical store")

        for edge_key, edge_rec in self.edge_store.items():
            # Handle both new canonical EdgeRec and legacy EdgeRec
            if hasattr(edge_rec, 'owner_net'):
                # New canonical EdgeRec with single owner
                if edge_rec.owner_net is None:
                    continue
                net_id = edge_rec.owner_net
            elif hasattr(edge_rec, 'owners'):
                # Legacy EdgeRec with multiple owners (fallback)
                if not edge_rec.owners:
                    continue
                net_id = list(edge_rec.owners)[0]  # Take first owner
            else:
                # Unknown EdgeRec type - skip
                continue
            ...
            # Extract edge coordinates from canonical key
            ...
            # Create track segment using canonical coordinates
            track = {
                'net_id': net_id,
                'layer': layer_id,
                'start': (x1, y1),
                'end': (x2, y2),
                'width': self.geometry.track_width,  # Default track width
                'source': 'canonical_edge_store'  # Tag for debugging
            }
            intents.tracks.append(track)
            kept_segments += 1

        logger.info(f"[CANONICAL-EMIT] Created {len(intents.tracks)} tracks from canonical edge store")
        ...
        if kept_segments == 0 and len(self.edge_store) > 0:
    ...
    def canon_edge_key(self, i1: int, i2: int, layer_id: int) -> tuple:
        ...
        # Convert numpy types to Python int (CRITICAL FIX)
        i1 = int(i1)
        i2 = int(i2)
    ...
    def rip_up_net(self, net_id: str, path_node_indices: list = None):
        """
        Rip up net by scanning canonical edge store directly - SINGLE SOURCE OF TRUTH
        """
        # Scan canonical edge store directly for this net's edges
        ripped_edges = 0
        current_iter = getattr(self, 'current_iteration', 0)

        # Scan all edges in the canonical store to find ones owned by this net
        for edge_key, edge_rec in self.edge_store.items():
            # Handle both new canonical EdgeRec and legacy EdgeRec
            if hasattr(edge_rec, 'owner_net'):
                if edge_rec.owner_net == net_id:
                    # Clear ownership
                    edge_rec.owner_net = None
                    # Add taboo restriction to prevent immediate reclaim
                    edge_rec.taboo_until_iter = current_iter + 1
                    # Increment historical cost for negotiation
                    edge_rec.historical_cost += 1.0
                    ripped_edges += 1
            elif hasattr(edge_rec, 'owners') and edge_rec.owners:
                if net_id in edge_rec.owners:
                    # Legacy EdgeRec - clear from owners set
                    edge_rec.owners.discard(net_id)
                    # Add taboo restriction to prevent immediate reclaim
                    if hasattr(edge_rec, 'taboo_until_iter'):
                        edge_rec.taboo_until_iter = current_iter + 1
                    # Increment historical cost for negotiation
                    if hasattr(edge_rec, 'historical_cost'):
                        edge_rec.historical_cost += 1.0
                    ripped_edges += 1

        if ripped_edges == 0:
            logger.warning(f"[RIPUP] net={net_id}: no edges found in canonical store to rip up")
        else:
            logger.info(f"[RIPUP] net={net_id}: ripped {ripped_edges} edges from canonical store with taboo restrictions")
    ...
    def _pathfinder_cost_for_canonical_edge(self, edge_key: tuple, net_id: str) -> float:
        ...
        # Ensure EdgeRec has required attributes for compatibility
        if not hasattr(rec, 'owner_net'):
            rec.owner_net = None
            rec.taboo_until_iter = 0
            rec.historical_cost = 0.0
    ...
    def commit_net_path(self, net_id: str, path_node_indices: list):
        ...
        """
        logger.info(f"[COMMIT] Starting commit for net={net_id} with path length={len(path_node_indices) if path_node_indices else 0}")

        if not path_node_indices or len(path_node_indices) < 2:
            logger.warning(f"[COMMIT] net={net_id}: path too short, no edges to commit")
        ...
        for i in range(len(path_node_indices) - 1):
            ...
            try:
                logger.debug(f"[COMMIT] Processing edge {i}: {from_idx} -> {to_idx}")

                # Get coordinates for layer check and spatial indexing
                from_coord = self._idx_to_coord(from_idx)
                to_coord = self._idx_to_coord(to_idx)
                logger.debug(f"[COMMIT] Coords: {from_idx}={from_coord}, {to_idx}={to_coord}")

                if not from_coord or not to_coord:
                    logger.debug(f"[COMMIT] Skipping - invalid coordinates")
                    continue

                x1, y1, layer1 = from_coord
                x2, y2, layer2 = to_coord
                logger.debug(f"[COMMIT] Edge: ({x1},{y1},L{layer1}) -> ({x2},{y2},L{layer2})")

                # Handle via edges separately (different layers)
                if layer1 != layer2:
                    # This is a via edge - store via separately
                    logger.debug(f"[COMMIT] Via edge detected: {from_idx}->{to_idx} L{layer1}->L{layer2}")
                    continue  # Skip vias for now, handle only track segments

                # Create canonical edge key using node indices (CRITICAL FIX)
                logger.debug(f"[COMMIT] Creating canonical key for nodes {from_idx}, {to_idx}")
                k = self._canon_edge_key_from_nodes(from_idx, to_idx)
                logger.debug(f"[COMMIT] Canonical key: {k}")

                # Get/create edge record
                if k not in self.edge_store:
                    self.edge_store[k] = EdgeRec()
                    logger.debug(f"[COMMIT] Created new EdgeRec for key {k}")
                else:
                    logger.debug(f"[COMMIT] Using existing EdgeRec for key {k}")

                rec = self.edge_store[k]

                # Handle compatibility with different EdgeRec versions
                if not hasattr(rec, 'owner_net'):
                    rec.owner_net = None
                    rec.taboo_until_iter = 0
                    rec.historical_cost = 0.0
                    logger.debug(f"[COMMIT] Initialized EdgeRec attributes for key {k}")
                ...
                logger.debug(f"[COMMIT] Successfully added edge {k} to net {net_id}")

                # Add to R-tree spatial index for clearance checking
                self._add_edge_to_rtree(net_id, layer1, x1, y1, x2, y2)

            except Exception as e:
                logger.error(f"[COMMIT] EXCEPTION processing edge {from_idx} -> {to_idx}: {e}")
                import traceback
        ...
        logger.info(f"[COMMIT] net={net_id}: committed {len(edge_keys_for_net)} edges to canonical store")
