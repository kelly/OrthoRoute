    ...
    def _find_zone_connected_groups(self, board, pins, net_code):
        ...
        for zone in zones:
            ...
            for pin in ungrouped_pins:
                ...
                pin_in_zone = False
                try:
                    # Method 1: Try HitTestFilledArea (newer KiCad versions)
                    if hasattr(zone, 'HitTestFilledArea'):
                        try:
                            pin_in_zone = zone.HitTestFilledArea(layer, pad_pos, 0)
                        except Exception as e:
                            print(f"⚠️ HitTestFilledArea failed: {e}")
                            pin_in_zone = False
                    
                    # Method 2: Try simple bounding box test (most compatible)
                    if not pin_in_zone:
                        try:
                            bbox = zone.GetBoundingBox()
                            pin_in_zone = bbox.Contains(pad_pos)
                            print(f"🔍 Using bounding box zone detection for pin at ({pin['x']}, {pin['y']})")
                        except Exception as e:
                            print(f"⚠️ Bounding box test failed: {e}")
                            pin_in_zone = False
                        
                except Exception as e:
                    print(f"❌ All zone hit tests failed for pin at ({pin['x']}, {pin['y']}): {e}")
                    pin_in_zone = False
